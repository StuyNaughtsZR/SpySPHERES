...
void init(){
	...
	DEBUG(("Stuy-Naught! Modified With Mirror"));
}
...
void loop(){
	...
	switch(state) {
	    case START:
	        if(!(itemBool[7] && itemBool[8]))
	            {itemNum = recalibrate(4,6);
	            state = MAIN;
	            break;} 
	            
	        if(game.posInLight(me)&&game.posInLight(other))
	            {itemNum = recalibrate(4,6);
	            state = MAIN;}
	    ...
	    case MAIN: //normal case
	        ...
	        if (game.posInLight(me)&&game.getOtherEnergy()>.9
	            &&game.getLightSwitchTime()>4&&(
	            !game.posInLight(other)
...
void openFire(){
    if (game.getMirrorTimeRemaining()<1){
    	mathVecSubtract(facing, other, me, 3);
    	api.setAttitudeTarget(facing);
