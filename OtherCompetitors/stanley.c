...
void init(){
	...
	me[0] = me[1] = me[2] = myPos[0] = myPos[1] = myPos[2] = 0.0f;
	
	other[0] = other[1] = other[2] = otherPos[0] = otherPos[1] = otherPos[2] = 0.0f;
...
void loop(){
	...
	aboveOtherPos[0] = otherPos[0] = other[0];
	aboveOtherPos[1] = otherPos[1] = other[1];
    ...
    if (game.getMemoryFilled() == 2) {
        ...
        if (matrixEqual(myAtt, earth) && game.getEnergy() >= 3) {
            game.uploadPics();
        }
    }
    ...
    switch (state) {
        case 0://Get items
            ...
            /*else if (game.hasItem(3) == -1) {
                ...
                else api.setPositionTarget(myPos);
            }*/
            ...
            else if (game.hasItem(4) == -1 && !(sphColor)) {
                if (game.getEnergy() > 3) moveFast(score2);
                else api.setPositionTarget(myPos);
            }
            else if (game.hasItem(5) == -1 && sphColor) {
...
void moveFast(float target[3]) {
    if (dist(myPos, target) > 0.49) {
        mathVecSubtract(vecBtwn, target, myPos, 3);
...
bool canTakePic() {
    return (game.posInLight(otherPos) || game.posInGrey(otherPos)) && (dist(myPos, otherPos) > 0.5) && game.isFacingOther() && game.isCameraOn();
}
