...
void loop() {
    ...
    switch(state) {
        case getInPosForPic: // if other in light, point towards other
            ...
            else if (firstTimePic) {
                mathVecSubtract(facing, me, other, 3);
                mathVecNormalize(facing, 3);
                mathVecMultiply(facing, 0.55);
        ...
        case getItem: // if other in dark, go for the point item
            targetID = minDistEl(scores, me, 4);
            DEBUG(("TARGET ID : %d\n", targetID));
            DEBUG(("TARGET LOC : %f, %f, %f\n", scores[targetID][0], scores[targetID][0], scores[targetID][0]));
            DEBUG(("ACTUAL LOC : %f, %f, %f\n", game.getItem()))
            if (targetID == -1) {
                state = getEnergyState;
                break;
            }
            ...
            if (areWeThereYet(target, me)) {
                ...
                if (game.hasItem(targetID + 3) != -1) {
                    scores[targetID][0] = 99;
                    scores[targetID][1] = 99;
                    scores[targetID][2] = 99;
                }
            }
            else {
        ...
        case getEnergyState: // if low battery, go grab energy pack
            targetID = minDistEl(energy, me, 3);
            if (targetID == -1) {
                api.setPositionTarget(me);
                break;
            }
            ...
            if (areWeThereYet(target, me)) {
                ...
                if (game.hasItem(targetID) != -1) {
                    energy[targetID][0] = 99;
                    energy[targetID][1] = 99;
                    energy[targetID][2] = 99;
                }
            }
            else {
                api.setPositionTarget(target);
